...
pred.SSC.int <-
  crossing(
    ln_DoE = seq(from = -4, to = 10, by = 0.01),
    draw = 1:10000
  ) %>%
...
pred.SSC.int.quant <- pred.SSC.int %>%
  group_by(ln_DoE) %>%
  summarise(
    lower5 = quantile(pred.ln.SSC, probs = 0.05, na.rm = TRUE),
...
pred.SSC.no_int.quant <- pred.SSC.no_int %>%
  group_by(ln_DoE) %>%
  summarise(
    lower5 = quantile(pred.ln.SSC, probs = 0.05, na.rm = TRUE),
    upper5 = quantile(pred.ln.SSC, probs = 0.95, na.rm = TRUE)
...
pred.prob.int <-
  crossing(
    ln_SSC = seq(from = -1, to = 12, by = 0.01),
    draw = 1:10000
  ) %>%
  left_join(dat1.int, by = "draw")
...
pred.prob.int <- pred.prob.int %>%
  mutate(
    pred.logit.low = alpha + ln_SSC * beta.SSC + mean(filter(m.data, ln.DoE_bin == "Low")$ln.DoE) * beta.DoE + ln_SSC * mean(filter(m.data, ln.DoE_bin == "Low")$ln.DoE) * beta.int,
...
pred.prob.int.q95 <- pred.prob.int %>%
  group_by(ln_SSC) %>%
  summarise(
    pred.logit.low.95lower = quantile(pred.logit.low, probs = 0.025, na.rm = TRUE),
...
pred.mean.int <-
  crossing(
    ln.SSC = seq(from = -1, to = 12, by = 0.01),
    ln.DoE = c(
      mean(filter(m.data, ln.DoE_bin == "Low")$ln.DoE),
...
p.logit.DoE.low.int <- ggplot() +
  theme_classic() +
  geom_jitter(
    data = filter(m.data, ln.DoE_bin == "Low"),
    mapping = aes(
      x = ln.SSC,
      y = (Effect1 - 0.5) * 100,
      shape = as.factor(Effect1)
...
p.logit.DoE.med.int <- ggplot() +
  theme_classic() +
  geom_jitter(
    data = filter(m.data, ln.DoE_bin == "Medium"),
    mapping = aes(
      x = ln.SSC,
      y = (Effect1 - 0.5) * 100,
      shape = as.factor(Effect1)
...
p.logit.DoE.high.int <- ggplot() +
  theme_classic() +
  geom_jitter(
    data = filter(m.data, ln.DoE_bin == "High"),
    mapping = aes(
      x = ln.SSC,
      y = (Effect1 - 0.5) * 100,
      shape = as.factor(Effect1)
...
pred.prob.no_int <- pred.prob.no_int %>%
  mutate(
    pred.logit.low = alpha + ln_SSC * beta.SSC + mean(filter(m.data, ln.DoE_bin == "Low")$ln.DoE) * beta.DoE,
    pred.prob.low = 1 / (1 + exp(-pred.logit.low)),
...
dose <- tibble(
  DoE = seq(from = 0.1, to = 48, by = 0.001),
  SSC.10 = 24 * 10 / DoE,
  SSC.25 = 24 * 25 / DoE,
  SSC.50 = 24 * 50 / DoE,
  SSC.100 = 24 * 100 / DoE,
